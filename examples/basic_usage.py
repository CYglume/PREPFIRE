...
pipeline = prepfire.PrepFirePipeline(
...
# Run individual steps with method chaining (skipping weather download)
pipeline.process_fire_data() \
...
print("\nGenerated files:")
print(f"- Processed fire data: {pipeline.fires_gdf.shape[0]} fires processed")
print(f"- Weather types: {len(pipeline.fire_weather)} weather records")
print(f"- Number of clusters: {pipeline.km_model.n_clusters}")
print(f"- LCP file: {pipeline.lcp_file}")
